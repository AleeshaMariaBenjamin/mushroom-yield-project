# Data Quality Report

## Dataset Overview

Total observations: ______

Date range: ______ to ______

## Summary Statistics

(Add output from df.describe())

## Data Quality Metrics

- Missing values handled during cleaning
- Invalid values removed or corrected

## Insights

1. Humidity remains within expected operating range.
2. Yield values show sufficient variation for prediction.
3. Mean and median values are similar, indicating limited skewness.

Summary Statistics
       temperature   humidity         CO2      yield
count    60.000000  60.000000   60.000000  60.000000
mean     25.468333  85.133333  766.300000   1.383333
std       0.533772   2.054071   27.461714   0.106033
min      24.400000  82.000000  715.000000   1.200000
25%      25.075000  83.000000  747.250000   1.300000
50%      25.500000  85.000000  769.000000   1.400000
75%      25.900000  87.000000  790.000000   1.500000
max      26.400000  89.000000  812.000000   1.600000